...
// AcquireACLPathHandle is the Windows elevated-grant variant of
// AcquirePathHandle. It acquires READ_CONTROL/WRITE_DAC authority during the
// identity-validation open itself, so a later compiler never has to discard
// the validated handle and reopen an attacker-swappable path.
func AcquireACLPathHandle(binding *PathBinding, target string, exact bool) (*PathHandle, error) {
	if binding == nil || !winpath.EqualPath(binding.CanonicalPath, target) ||
		!winpath.EqualPath(binding.ExistingPath, target) {
		return nil, ErrTargetChanged
	}
	object, err := winpath.OpenForACL(target, !exact, !exact)
	if err != nil {
		return nil, fmt.Errorf("%w: %v", ErrTargetChanged, err)
	}
	identity := windowsObjectIdentity(object)
