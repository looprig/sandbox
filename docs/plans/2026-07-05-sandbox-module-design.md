...
subagent/foreign-agent section, acceptance matrix).
...
- **Leaf module: stdlib + `golang.org/x/sys` only.** No looprig imports — not even
  `core`. It must be independently useful (sandbox any `exec.Cmd` in any Go
...
- Writable roots carry protected carveouts: `.git`, `.looprig` (and configurable
  ...
  through the gate.
- Tmp: `/tmp` and `$TMPDIR` writable in `write`+, excludable per policy.
...
to the policy's writable tmp. Everything else — `SSH_AUTH_SOCK`, `AWS_*`,
...
| 2 | Landlock v4+ (FS + TCP port rules) + pure-Go seccomp-BPF (UDP socket denial, ptrace/io_uring denial) + `PR_SET_NO_NEW_PRIVS`; no namespaces | FS by enumerated allowlist (§7.5), TCP port allowlist; no address scoping, no restricted-read |
...
### 10.3 Level interlock

The checker probes its configured runner for `interface{ Level() uint8 }`:
**auto-approve-bash postures activate only when `Level() ≥ threshold`**,
default threshold `LevelFull` (`LevelExternal` qualifies only because the
deployment explicitly constructed it). Consumers may consciously lower the
...
1. `$TMPDIR` resolving outside `/tmp` (macOS `/var/folders/...`): include as
   writable? Proposed: yes, with deny-glob carveouts.
2. Triviality classifier for `write` mode: reuse/extend existing
   `HardApproveRules` prefix rules, or adopt an execpolicy-style argv rule
   language? Proposed: prefix rules in v1.
3. `trusted` default `Ports {443}` — also 80? Proposed: no; http:// is gated.
4. Name and shape of the swe-level knob (`WithSecurityMode` vs per-role map).
5. Grant TTL default (proposed 15 min) and whether `ScopeSession` match keys
   should include the delta at path granularity or capability granularity.
6. Seatbelt CIDR/network-filter expressiveness — verify during implementation;
   determines whether macOS `trusted` gets `Private`/metadata semantics or
   compiles them to blocked.
