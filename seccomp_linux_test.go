...
func requireSeccomp(t *testing.T) {
	...
	if !probeSeccompFilter() {
		t.Skip("SECCOMP_MODE_FILTER unavailable on this host; rung-2 seccomp filter cannot run")
	}
}

// --- Structural proof: a tiny classic-BPF interpreter over buildSeccompFilter() -
//
...
func seccompData(arch uint32, nr uint32, args [6]uint64) []byte {
	buf := make([]byte, 64)
	binary.LittleEndian.PutUint32(buf[seccompOffNR:], nr)
	binary.LittleEndian.PutUint32(buf[seccompOffArch:], arch)
	for i := 0; i < 6; i++ {
...
// runBPF executes the subset of classic BPF that buildSeccompFilter() uses
...
func TestBuildSeccompFilterStructure(t *testing.T) {
	...
	tests := []struct {
		...
		{"x32 socket killed", x86, unix.SYS_SOCKET | seccompX32SyscallBit, [6]uint64{unix.AF_INET, dgram, 0}, kill},
		{"x32 write killed", x86, 1 | seccompX32SyscallBit, [6]uint64{}, kill},
		// UDP denials.
		...
		// MPTCP denials.
		{"AF_INET stream MPTCP denied", x86, unix.SYS_SOCKET, [6]uint64{unix.AF_INET, stream, uint64(ipprotoMPTCP)}, retErrno},
		{"AF_INET6 stream MPTCP denied", x86, unix.SYS_SOCKET, [6]uint64{unix.AF_INET6, stream, uint64(ipprotoMPTCP)}, retErrno},
		{"AF_INET stream MPTCP with CLOEXEC denied", x86, unix.SYS_SOCKET, [6]uint64{unix.AF_INET, stream | cloexec, uint64(ipprotoMPTCP)}, retErrno},
		// nr-only denials.
...
// carries stage2SentinelEnv, the distinguisher checked below).
...
func seccompTargetDispatch() {
	...
	fmt.Printf("%s=%s\n", seccompKeyMPTCP, classifySeccompSocket(unix.AF_INET, unix.SOCK_STREAM, ipprotoMPTCP))
...
// linuxBackend stage-2 (Landlock + seccomp), a post-execve target has UDP, MPTCP,
...
func TestLinuxSeccompDeniesInTarget(t *testing.T) {
	requireLandlockV4(t) // rung-2 backend also needs Landlock v4 to spawn
	requireSeccomp(t)
	...
	checks := []struct {
		...
		{"UDP socket denied (no address scoping at rung 2)", seccompKeyUDP, seccompValEACCES},
